...
const ConfiguracionPage = () => {
  ...
  const handleInviteMember = async () => {
    ...
    setInviting(true);
    try {
      await axios.post(`${API}/organizacion/miembros`, {
      ...
      toast.error(error.response?.data?.detail || "Error al invitar miembro");
    } finally {
      setInviting(false);
    }
  };
  ...
  const handleChangeRole = async (userId, newRole) => {
    try {
      await axios.put(`${API}/organizacion/miembros/${userId}/role`, {
        user_email: "",
        role: newRole
      }, { withCredentials: true });
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Gestiona multiples organizaciones desde una sola cuenta
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <Dialog open={newOrgDialogOpen} onOpenChange={setNewOrgDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Crea una nueva donataria autorizada. Los datos se pueden completar despues en Configuracion.
                  </DialogDescription>
                </DialogHeader>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Gestiona quien tiene acceso a esta organizacion y su nivel de permisos
                  </CardDescription>
                </div>
              </div>
              {myRole === "admin" && (
                <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
                  <DialogTrigger asChild>
                    <Button size="sm" className="bg-violet-600 hover:bg-violet-700" data-testid="invite-member-btn">
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        El usuario debe tener una cuenta registrada en DonatariaSAT.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      {myRole === "admin" ? (
                        <Select
                          ...
                          disabled={member.email === members.find(m => m.role === 'admin')?.email && members.filter(m => m.role === 'admin').length <= 1}
                        >
                          <SelectTrigger className="w-28 h-8 text-xs" data-testid={`role-select-${member.user_id}`}>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                        ...
                        <Badge variant="outline" className={`text-xs ${
                          member.role === 'admin' ? 'border-emerald-200 text-emerald-700 bg-emerald-50' :
                          member.role === 'editor' ? 'border-blue-200 text-blue-700 bg-blue-50' :
